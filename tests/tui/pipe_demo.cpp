...
Tag parse(const std::string& raw) {
    ...
    size_t sp = raw.find_first_of(" >/", s);
    t.name = raw.substr(s, sp - s);
    if (sp != std::string::npos && raw[sp] == ' ') {
        for (size_t p = sp + 1; p < raw.size();) {
            ...
            while (!k.empty() && k.back() == ' ')
                k.pop_back();
            size_t vs = raw.find('"', e);
            if (vs == std::string::npos)
                break;
            vs++;
            size_t ve = raw.find('"', vs);
            if (ve == std::string::npos)
                break;
            t.a[k] = raw.substr(vs, ve - vs);
            p = ve + 1;
        }
    }
    return t;
...
int main(int argc, char** argv) {
    ...
    if (argc > 1) {
        ...
        while (std::getline(f, ln)) {
            if (ln.find("<stream>") != std::string::npos)
                inStream = true;
            else if (ln.find("</stream>") != std::string::npos)
                break;
            else if (inStream)
                input += ln + "\n";
        }
    ...
    if (env)
        delayMs = std::stoi(env);
    if (argc > 2)
        delayMs = std::stoi(argv[2]);

    auto render = [&] {
        for (auto& l : pv.incremental(60))
            std::cout << "  " << l << "\n";
        std::cout << std::flush;
    };
    ...
    for (size_t i = 0; i < input.size(); i++) {
        ...
        if (tok[0] == '<' && tok.back() == '>') {
            ...
            if (!t.close) {  // Opening
                ...
                if (t.name == "action") {
                    ...
                    if (t.a["type"] == "agent")
                        at = ActionType::AGENT;
                    else if (t.a["type"] == "relic")
                        at = ActionType::RELIC;
                    else if (t.a["type"] == "feed")
                        at = ActionType::FEED;
        ...
        // Body text (inside open tag or split body+close)
            ...
            if (ct != std::string::npos && tok.back() == '>') {
                body += tok.substr(0, ct);
                auto ct2 = parse(tok.substr(ct));
                done = true;
