...
| `bc81a89` | AGENT header `mode`/`eph`/`tNs` + child well engine/iter |
...
- `runLoop` is **~976 lines** (`agent.cpp:293–1269`). `steerIncompleteGeneration` + `classifyTransportError` extracted. Next: `TransportCaller` (the generateStream catch). `TurnState` stack still not done — events still on `Agent`.
...
- **Stop:** `g_hardKill` = SIGINT/SIGTERM only. TUI Ctrl-X = `Agent::requestStop` (walks children). Stream stall = `RunControl::clearSoft()` on **this** agent. Never `g_hardKill=false` from stall recovery.
- Fallback provider restored at `prompt()` exit (`restorePrimaryIfFallback`). Sticky swap was a bug.
