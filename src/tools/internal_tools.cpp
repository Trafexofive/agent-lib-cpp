...
static std::string toolFsWrite(const Json::Value& p) {
    ...
    if (lastSlash != std::string::npos) {
        std::string d;
        runCmd("mkdir -p " + shellEscape(path.substr(0, lastSlash)), d, 5);
    }
...
static std::string toolContextPin(const Json::Value& p) {
    ...
    std::string d;
    runCmd("mkdir -p " + shellEscape(md), d, 5);
    std::string bn = path.substr(path.find_last_of('/') + 1);
    {
        std::ofstream mc(md + bn + ".content");
        mc << content;
    }
    {
        std::ofstream mm(md + bn + ".meta");
        mm << path;
    }
    Json::Value r;
    r["success"] = true;
    r["path"] = path;
    r["size"] = (int)content.size();
    r["mode"] = "pinned";
    return jsonStr(r);
}
...
static std::string toolContextUnpin(const Json::Value& p) {
    ...
    if (std::filesystem::exists(md + bn + ".content")) {
        ...
        std::filesystem::remove(md + bn + ".meta");
        removed = true;
    }
    Json::Value r;
    r["success"] = removed;
