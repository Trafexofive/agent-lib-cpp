...
std::string SessionManager::iso8601() {
    return nowISO();
}
...
static SessionRecord::Role parseRole(const std::string& s) {
    ...
    if (s == "agent")
        return SessionRecord::AGENT;
    if (s == "tool_call")
        return SessionRecord::TOOL_CALL;
    if (s == "tool_result")
        return SessionRecord::TOOL_RESULT;
...
void SessionManager::prune(const std::string& id, size_t maxRecords) const {
    Session s = load(id);
    if (s.records.size() <= maxRecords)
        return;
    size_t remove = s.records.size() - maxRecords;
    if (!s.records.empty())
        remove = std::min(remove, s.records.size() - 1);
...
bool SessionManager::exportToFile(const std::string& id, const std::string& path) const {
    Session s = load(id);
    if (s.records.empty() && s.id.empty())
        return false;
    Json::Value root;
    root["format"] = "cortex-session-portable";
    root["version"] = "1.0";
    root["id"] = s.id;
    root["agent"] = s.agentName;
    root["model"] = s.model;
    root["provider"] = s.provider;
    root["created"] = s.created;
    root["updated"] = s.updated;
...
std::vector<SessionRecord> SessionManager::importLegacyHistory(const std::vector<std::string>& h) {
    ...
    for (auto& line : h) {
        ...
        if (line.rfind("User:", 0) == 0) {
            r.role = SessionRecord::USER;
            r.content = line.substr(6);
        } else if (line.rfind("Agent:", 0) == 0) {
            r.role = SessionRecord::AGENT;
            r.content = line.substr(7);
        } else if (line.rfind("System (Action Result):", 0) == 0) {
            r.role = SessionRecord::TOOL_RESULT;
            r.content = line.substr(23);
        } else if (line.rfind("System:", 0) == 0) {
            r.role = SessionRecord::SYSTEM;
            r.content = line.substr(8);
        } else {
            r.role = SessionRecord::SYSTEM;
            r.content = line;
        }
        // trim
        while (!r.content.empty() && r.content[0] == ' ')
            r.content.erase(0, 1);
        recs.push_back(r);
    }
    return recs;
...
std::vector<std::string> SessionManager::exportLegacyHistory(
    const std::vector<SessionRecord>& recs) {
    std::vector<std::string> h;
    for (auto& r : recs) {
        switch (r.role) {
            case SessionRecord::USER:
                h.push_back("User: " + r.content);
                break;
            case SessionRecord::AGENT:
                h.push_back("Agent: " + r.content);
                break;
            case SessionRecord::TOOL_CALL:
                h.push_back("Tool Call: " + r.content);
                break;
            case SessionRecord::TOOL_RESULT:
                h.push_back("System (Action Result): " + r.content);
                break;
            default:
                h.push_back("System: " + r.content);
                break;
        }
    }
    return h;
