   ...
   public:
    struct Theme {
        ...
        static Theme defaultTheme() {
            ...
            t.heading = ansi::bold();
            t.bold = ansi::bold();
            t.italic = ansi::italic();
            t.code = ansi::dim();
            t.codeBlock = ansi::dim();
            t.quote = ansi::dim();
            t.link = ansi::underline();
            t.listBullet = ansi::dim();
            t.hr = ansi::dim();
            t.reset = ansi::reset();
            t.dim = ansi::dim();
            return t;
    ...
    std::vector<std::string> render() const {
        if (cached_ && cached_width_ == width_)
   ...
   private:
    ...
    std::vector<std::string> renderImpl(const std::string& text, int width) const {
        ...
        while (std::getline(iss, line)) {
            ...
            if (isFence(line)) {
                ...
                if (!lang.empty())
                    lines.push_back(ansi::dim() + ansi::fg(0, 150, 200) + " ── " + lang + " ──" +
                                    ansi::reset());
                continue;
            }
            ...
            // ── Header ──
            if (isHeader(line)) {
                auto hl = renderHeader(line);
                lines.insert(lines.end(), hl.begin(), hl.end());
                continue;
            }
            ...
            if (isBlockquote(line)) {
                lines.push_back(renderBlockquote(line));
                continue;
            }
    ...
    std::vector<std::string> renderHeader(const std::string& line) const {
        ...
        if (level == 1) {
            ...
            result.push_back(ansi::dim() + std::string(std::min((size_t)width_, vis + 4), '-') +
                             ansi::reset());
        } else if (level == 2) {
            result.push_back(ansi::bold() + ansi::cyanText(text) + ansi::reset());
            result.push_back(ansi::dim() + std::string(std::min((size_t)width_ / 2, vis), '-') +
                             ansi::reset());
        } else if (level >= 3) {
    ...
    std::string renderUnorderedList(const std::string& line) const {
        ...
        if (content.rfind("[x] ", 0) == 0 || content.rfind("[X] ", 0) == 0) {
            return std::string(indent, ' ') + ansi::fg(0, 200, 0) + "[✓]" + ansi::reset() + " " +
                   renderInline(content.substr(4));
        }
        if (content.rfind("[ ] ", 0) == 0) {
            return std::string(indent, ' ') + ansi::dim() + "[ ]" + ansi::reset() + " " +
                   renderInline(content.substr(4));
        }
        return std::string(indent, ' ') + ansi::dim() + bullet + " " + ansi::reset() +
    ...
    std::string renderOrderedList(const std::string& line) const {
        ...
        return std::string(indent, ' ') + ansi::dim() + number + ". " + ansi::reset() +
               renderInline(content);
    }
    ...
    std::string renderInline(const std::string& line) const {
        ...
        result = std::regex_replace(result, strikeRe,
                                    std::string(ansi::strikethrough()) + "$1" + theme_.reset);
        // Math: $inline$ → dim italic
        static const std::regex mathRe(R"(\$([^$]+?)\$)");
        result =
            std::regex_replace(result, mathRe, ansi::dim() + ansi::italic() + "$1" + ansi::reset());
        // Links: [text](url) → text (url)
        static const std::regex linkRe(R"(\[([^\]]+)\]\(([^)]+)\))");
        result = std::regex_replace(
            result, linkRe,
            theme_.bold + "$1" + theme_.reset + " " + theme_.dim + "($2)" + theme_.reset);
        // Images: ![alt](url) → 🖼 alt
        static const std::regex imgRe(R"(!\[([^\]]*)\]\(([^)]+)\))");
        result = std::regex_replace(result, imgRe, theme_.dim + "🖼 $1" + theme_.reset);
        if (inlineCache_.size() > 512)
            inlineCache_.clear();
        ...
        while (pos < text.size()) {
            if (text[pos] == '\033') {
                ...
                if (end != std::string::npos) {
                    current += text.substr(pos, end - pos + 1);
                    pos = end + 1;
                } else {
                    current += text[pos++];
                }
            ...
            if (visibleLen + wv + (visibleLen > 0 ? 1 : 0) > (size_t)width && visibleLen > 0) {
                flush();
                current = word;
                visibleLen = wv;
            } else {
    ...
    static std::vector<std::string> split(const std::string& s, char d) {
        ...
        size_t st = 0, e = s.find(d);
        while (e != std::string::npos) {
            r.push_back(s.substr(st, e - st));
            st = e + 1;
            e = s.find(d, st);
        }
        r.push_back(s.substr(st));
        return r;
    }
