...
struct ContentBlock {
    BlockType type = BlockType::RESPONSE;
    std::string text;
    bool isFinal = false;
};
...
class ProtocolView {
   public:
    void addAction(const ActionEvent& a) {
    ...
    void appendRenderedBlocks() {
        for (size_t i = lastBlock_; i < renderedBlocks_.size(); i++) {
            cached_lines_.insert(cached_lines_.end(), renderedBlocks_[i].begin(),
                                 renderedBlocks_[i].end());
        }
   ...
   private:
        ...
        if (body[0] == '{') {
            ...
            if (Json::parseFromStream(b, js, &root, &errs) && root.isObject()) {
                for (auto& key : root.getMemberNames()) {
                    ...
                    lines.push_back(ansi::fg(50, 200, 200) + fgBold() + key + fgReset() + fgDim() +
                                    ":  " + fgReset() + fgDim() + val + fgReset());
                }
                return lines;
            }
        }
        std::string c = body;
        if (c.size() > (size_t)(width - 8))
            c = c.substr(0, width - 11) + "...";
        lines.push_back(fgDim() + c + fgReset());
        return lines;
    }

    static std::string summarizeParamValue(const Json::Value& v, const std::string& key,
                                           int width) {
        if (v.isObject())
            return "{...}";
        if (v.isArray())
    ...
    std::vector<std::string> resultLines(const ResultEvent& r, int width) const {
        ...
        if (r.elapsedMs > 0 || r.exitCode != 0 || r.outputBytes > 0) {
            ...
            if (r.exitCode != 0)
                meta << ansi::fg(200, 50, 0) << "exit:" << r.exitCode << fgDim() << " ";
            if (r.outputBytes > 0) {
        ...
        if (!r.ok && (body.empty() || body == "?")) {
            lines.push_back(mark + ansi::fg(200, 50, 0) + "failed" + fgReset());
            return lines;
        }

        // Builtins (constant-time lookup)
        static const std::unordered_set<std::string> bi = {"bash", "exec",    "list",  "ls",
        ...
        if (!body.empty() && (body[0] == '{' || body[0] == '[')) {
            ...
            if (!yaml.empty()) {
                ...
                std::string yl;
                bool first = true;
                while (std::getline(ys, yl)) {
                    if (yl.empty())
                        continue;
                    lines.push_back((first ? mark : "  ") + yl);
                    first = false;
                }
                return lines;
            }
    ...
    std::vector<std::string> builtinLines(const ResultEvent& r, const std::string& mark,
                                          int width) const {
        std::vector<std::string> lines;
        auto& tn = r.toolName;
        std::string body = r.summary;
    ...
    static std::string jsonToYaml(const std::string& json, int indent) {
        ...
        if (!Json::parseFromStream(b, js, &root, &errs))
            return "";
        std::string out;
        jsonToYamlImpl(root, indent, out);
        return out;
    }

    static void jsonToYamlImpl(const Json::Value& v, int indent, std::string& out) {
        std::string pad(indent, ' ');
        auto keyS = [](const std::string& s) {
            return std::string(ansi::fg(50, 200, 200)) + fgBold() + s + fgReset();
        };
