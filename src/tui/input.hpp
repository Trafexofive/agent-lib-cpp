...
class Input {
   public:
        ...
        for (ssize_t i = 0; i < n; i++) {
            ...
            if (!escBuf_.empty()) {
                ...
                if (escBuf_.size() >= 2 && escBuf_[1] == '[') {
                    if (escBuf_.size() >= 3 && (unsigned char)c >= 0x40 &&
                        (unsigned char)c <= 0x7e) {
                        seq = escBuf_;
                        escBuf_.clear();
                        done = true;
                    } else if (escBuf_.size() > 12) {
                        escBuf_.clear();
                        continue;
                    } else
            ...
            if (searching() && act == KeyAction::CHAR) {
                extendSearch(outChar);
                continue;
            }
            if (searching() && (seq == "\x7f" || seq == "\x08")) {
                backspaceSearch();
                continue;
            }
            if (searching() && (act == KeyAction::CANCEL || act == KeyAction::EXIT)) {
                cancelSearch();
                continue;
            }
            if (searching() && seq == "\r") {
            ...
            switch (act) {
                ...
                case KeyAction::DELETE_FORWARD:
                    if (!suffix_.empty())
                        suffix_ = suffix_.substr(1);
                    break;
                case KeyAction::CURSOR_LEFT:
                    if (!buffer_.empty()) {
                        suffix_ = std::string(1, buffer_.back()) + suffix_;
                        buffer_.pop_back();
                    }
                    break;
                ...
                case KeyAction::CANCEL:
                    ...
                    break;
                case KeyAction::EXIT:
                    if (buffer_.empty() && suffix_.empty()) {
                        if (onEnter_)
                            onEnter_("/exit");
                        return true;
                    }
    ...
    bool searching() const {
        return !searchBuf_.empty();
    }
    std::string searchLine() const {
        return "(reverse-search)`'" + searchPattern_ + "': " + searchMatch_;
    }
    void startSearch() {
        if (!searchBuf_.empty()) {
            cycleSearch();
    ...
    void acceptSearch() {
        if (!searching())
            return;
        if (!searchMatch_.empty())
            buffer_ = searchMatch_;
        suffix_.clear();
        searchBuf_.clear();
        searchPattern_.clear();
        searchMatch_.clear();
    ...
    void updateSearch() {
        ...
        std::string lower = searchPattern_;
        for (auto& c : lower)
            c = std::tolower(c);
        int start = (searchPos_ >= 0 && searchPos_ < (int)entries.size()) ? searchPos_
                                                                          : (int)entries.size() - 1;
        for (int i = start; i >= 0; i--) {
            std::string el = entries[i];
            for (auto& c : el)
                c = std::tolower(c);
            if (el.find(lower) != std::string::npos) {
                searchMatch_ = entries[i];
                searchPos_ = i;
                return;
            }
        }
