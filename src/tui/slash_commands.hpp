...
static FrontMatter parseFrontMatter(const std::string& text) {
    ...
    if (out.description.empty())
        out.description = firstBodyLine(out.body);
    return out;
}
...
static SlashCommandEntry promptEntry(const std::filesystem::path& p) {
    ...
    if (!meta.argumentHint.empty())
        summary += " [arg: " + meta.argumentHint + "]";
    summary += " [prompt]";
...
static std::vector<SlashCommandEntry> libraryEntries() {
    ...
    if (std::filesystem::exists(skills)) {
        for (const auto& it : std::filesystem::recursive_directory_iterator(skills)) {
            if (!it.is_directory())
                continue;
            if (!std::filesystem::exists(it.path() / "SKILL.md"))
                continue;
...
static std::string commandKey(const std::string& cmd) {
    std::string k = cmd;
    if (k.rfind("/prompt/", 0) == 0)
        k.erase(0, 8);
    else if (k.rfind("/skill/", 0) == 0)
        k.erase(0, 7);
    else if (!k.empty() && k[0] == '/')
        k.erase(0, 1);
    return lower(k);
   ...
   public:
    ...
    static const SlashCommandEntry* find(const std::string& cmd) {
        ...
        for (const auto& e : entries()) {
            if (slash_detail::commandKey(e.name) == key)
                return &e;
        }
        return nullptr;
    }
    ...
    static std::vector<std::string> renderDynamic(const std::string& cmd) {
        ...
        if (!e->argumentHint.empty())
            lines.push_back("hint: " + e->argumentHint);
        lines.push_back("");
    ...
    static std::vector<std::string> helpLines() {
        ...
        lines.push_back("Scroll: Ctrl-O/Alt-K up, Ctrl-G/Alt-J down. Dialogs: j/k move, y/n confirm.");
