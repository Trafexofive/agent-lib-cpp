...
**CRITICAL: All output MUST use XML tags. Never output bare text outside tags. It is discarded.**

- Reasoning/planning → `<thought>your plan here</thought>` (hidden from user, kept in context)
- Non-final plan (visible) → `<response>your plan</response>` + actions in same turn
- Final answer → `<response final="true">answer</response>` (ends conversation)
